...
function GUI:init(services, config, callbacks)
    ...
    self.Window = Fluent:CreateWindow({
        ...
        Acrylic = false,
        IntegrateBackdrop = false,
        Theme = "Dark",
        MinimizeKey = Enum.KeyCode.RightShift
    })

    -- Adăugare Cursor Indicator
    local screenGui = Instance.new("ScreenGui",
        services.CoreGui or services.Players.LocalPlayer:WaitForChild("PlayerGui"))
    ...
    -- Funcția de update pentru cursor
    function GUI:updateCursorPosition(uis)
        if self.cursorIndicator and self.cursorIndicator.Visible then
            local mousePos = uis:GetMouseLocation()
            self.cursorIndicator.Position = UDim2.fromOffset(mousePos.X, mousePos.Y)
        end
    end

    self.Tabs = {
        Combat = self.Window:AddTab({
            Title = "Combat",
            Icon = "swords"
        }),
        Visuals = self.Window:AddTab({
            Title = "Visuals",
            Icon = "eye"
        }),
        Weapons = self.Window:AddTab({
            Title = "Weapons",
            Icon = "wrench"
        }),
    ...
    -- Combat
    self.Tabs.Combat:AddToggle("Silent", {
        Title = "Silent Aim",
        Default = config.sizingEnabled
    }):OnChanged(callbacks.onSizingToggle)
    self.Tabs.Combat:AddToggle("ShowTargetBox", {
        Title = "Show HitBox",
        Default = config.showTargetBox
    }):OnChanged(callbacks.onShowTargetBoxToggle)

    -- Visuals
    self.Tabs.Visuals:AddToggle("Walls", {
        Title = "Wall ESP",
        Default = config.highlightEnabled
    }):OnChanged(callbacks.onHighlightsToggle)
    ...
    self.Tabs.Visuals:AddDropdown("VisualStyle", {
        ...
        print("Style changed to: ", v)
    end)

    -- Weapons
    self.Tabs.Weapons:AddToggle("NoRecoil", {
        Title = "No Recoil",
        Default = config.patchOptions.recoil
    }):OnChanged(callbacks.onStabilityToggle)
